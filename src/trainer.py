    ...
    def train(self):
        for self.epoch in range(1, self.args.epochs+1):
            ...
            for _, sample in tqdm_bar:   
                ...
                X1_N = sample['vi_y'].to(device=self.dev)
                X2_N = sample['ir_y'].to(device=self.dev)
                ...
                vi_patch, ir_patch, fu_patch = self.dynamic_cropper(X1, X2, fusion_y, 32)

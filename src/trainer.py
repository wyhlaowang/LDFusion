...
class Trainer(object):
    def __init__(self,
        ...
        self.if_template = False
        self.if_mul_prompt = False
        self.if_resume = False

        self.step = 1 if platform == 'linux' else 1
        ...
        vi_text = ['a visible light gray image']
        ir_text = ['an infrared image']

        self.prompt = ['a vivid image with clear background and obvious objects']
        print(f'prompt: {self.prompt}')

        vit_template = compose_text_with_templates(vi_text, imagenet_templates)        
        vit_token = clip.tokenize(vit_template if self.if_template else vi_text).to(self.dev)
        ...
        irt_template = compose_text_with_templates(ir_text, imagenet_templates)
        irt_token = clip.tokenize(irt_template if self.if_template else ir_text).to(self.dev)
        ...
        prompt_template = compose_text_with_templates(self.prompt, imagenet_templates)
        prompt_token = clip.tokenize(prompt_template if self.if_template else self.prompt).to(self.dev)
        ...
        train_dataset = M3fdMix(self.prompt_feature.clone().cpu(), self.prompt_dir.clone().cpu())            
    ...
    def cropper_patch(self, im, num_crops=[24,16], size_l=[64,128]):
        im_re = im.repeat(1,3,1,1) if im.shape[1] == 1 else im
        cropper1 = transforms.Compose([transforms.RandomCrop(size_l[0]),
                                       transforms.RandomPerspective(0.5, 1, fill=0),
                                       transforms.Resize(224)])     
        cropper2 = transforms.Compose([transforms.RandomCrop(size_l[1]),
                                      transforms.RandomPerspective(0.5, 1, fill=0),
                                      transforms.Resize(224)])  
        im_patch1 = [cropper1(im_re) for _ in range(num_crops[0])]
        im_patch2 = [cropper2(im_re) for _ in range(num_crops[1])]
        return torch.cat(im_patch1+im_patch2, dim=0)

    def cropper_patch_multi(self, im_list, num_crops=[24,16], size_l=[64,128]):
        # im_list: [vi, ir, fusion]
        im_re = [(im.repeat(1,3,1,1) if im.shape[1] == 1 else im) for im in im_list]
        im_re = torch.cat(im_re, dim=0)
        cropper1 = transforms.Compose([transforms.RandomCrop(size_l[0]),
                                       transforms.RandomPerspective(0.5, 1, fill=0),
                                       transforms.Resize(224)])     
        cropper2 = transforms.Compose([transforms.RandomCrop(size_l[1]),
                                       transforms.RandomPerspective(0.5, 1, fill=0),
                                       transforms.Resize(224)])  
        im_patch1 = [torch.chunk(cropper1(im_re), chunks=3, dim=0) for _ in range(num_crops[0])]
        im_patch2 = [torch.chunk(cropper2(im_re), chunks=3, dim=0) for _ in range(num_crops[1])]
        vi_patch = []
        ir_patch = []
        fu_patch = []
        for ins1 in im_patch1:
            vi_patch.append(ins1[0])
            ir_patch.append(ins1[1])
            fu_patch.append(ins1[2])
        for ins2 in im_patch2:
            vi_patch.append(ins2[0])
            ir_patch.append(ins2[1])
            fu_patch.append(ins2[2])
        return torch.cat(vi_patch, dim=0), torch.cat(ir_patch, dim=0), torch.cat(fu_patch, dim=0)
    ...
    def clip_prompt_loss(self, fu_feature, vi_feature, ir_feature, prompt_feature, prompt_dir, if_foucs=False):
        ...
        if if_foucs:
            loss_prompt[loss_prompt < self.thresh] = 0
    ...
    def clip_sim_loss(self, fu_feature, vi_feature, ir_feature):
        ...
        loss_dir = 0.35 * (1 - torch.cosine_similarity(fusion_dir, vi_dir, dim=-1)) + \
                   0.65 * (1 - torch.cosine_similarity(fusion_dir, ir_dir, dim=-1))
    ...
    def vgg_loss(self, fu, vi, ir):
        ...
        loss_2 = 0.1 * F.mse_loss(fu_vgg['conv2_1'], feat2, reduction='mean')
        loss_3 = 0.4 * F.mse_loss(fu_vgg['conv3_1'], feat3, reduction='mean')
        loss_4 = 0.5 * F.mse_loss(fu_vgg['conv4_2'], feat4, reduction='mean')
    ...
    def train(self):
        for self.epoch in range(1, self.args.epochs+1):
            ...
            for _, sample in tqdm_bar:   
                ...
                X1_N = sample['vi_y_n'].to(device=self.dev)
                X2_N = sample['ir_y_n'].to(device=self.dev)
                ...
                fu_patch = self.cropper_patch(fusion_y)
                # vi_patch, ir_patch, fu_patch = self.cropper_patch_multi([X1, X2, fusion_y])
                ...
                # vi_patch_feature = self.clip_model.encode_image(self.clip_norm(vi_patch))
                # ir_patch_feature = self.clip_model.encode_image(self.clip_norm(ir_patch))
                ...
                # loss_prompt_p = self.clip_prompt_loss(fu_patch_feature, vi_patch_feature, ir_patch_feature, PF, PD, True)
                loss_prompt_p = self.clip_prompt_loss(fu_patch_feature, vi_feature, ir_feature, PF, PD, True)
                ...
                tv_loss = 0.002 * get_image_prior_losses(fusion_y)
                ...
                # tqdm update
                if GLOBAL_RANK == 0:
                    s = f'Train | sim:{loss_sim:.2f} | prompt:{loss_prompt:.2f} | percep:{perception_loss:.2f} | tv:{tv_loss:.2f} | total:{loss:.2f} | '

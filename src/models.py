...
class Encoder(nn.Module):
    def __init__(self, in_channels=3, dim=64, n_residual=3, n_downsample=2, style_dim=8):
        ...
        self.content_encoder2 = ContentEncoder(in_channels, dim, n_residual, n_downsample)
        self.style_encoder1 = StyleEncoder(1, dim, n_downsample, style_dim)
...
class Decoder(nn.Module):
    def __init__(self, out_channels=3, dim=64, n_residual=3, n_upsample=2, style_dim=16, atten_dim=16):
        ...
        self.model_global = CLA(in_channels=in_dim//2, embed_dim=64, out_channels=in_dim)
    ...
    def forward(self, vi_content_code, ir_content_code, vi_style_code, ir_style_code):
        ...
        feat_global = self.model_global(vi_content_code, ir_content_code)
        feat_local = self.model_local(feat_global)
